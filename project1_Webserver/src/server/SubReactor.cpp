...
#include "Server.h"
#include "HttpRequest.h"  // 新增：包含HttpRequest头文件
#include "Router.h"       // 新增：包含Router头文件
#include <arpa/inet.h>
#include <sys/epoll.h>
#include <fcntl.h>
#include <unistd.h>
...
#include <cstring>
#include <errno.h>
#include <cstdlib>        // 新增：包含cstdlib用于getenv函数
...
void SubReactor::handleRead(int cfd) {
    ...
    // 【关键修改】从epoll移除该fd的监听（因为ThreadPool将处理并关闭它）
    ...
    pool_.enqueue([this, cfd, buffer]() mutable {
        ...
        if (!request.parse(buffer)) {
            Router::sendError(cfd, 400, "Bad Request");
            close(cfd);
        ...
        bool handled = router.handleRequest(request, cfd, basePath);
        
        if (!handled) {
            Router::sendError(cfd, 404, "Not Found");
        }
    ...
    // 注意：不从connections_移除，等ThreadPool处理完再移除（在lambda中）
}
